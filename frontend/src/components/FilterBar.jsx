    ...
    <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 w-full bg-[var(--color-brand-surface)] p-4 sm:px-6 rounded-xl border border-[var(--color-brand-border)] shadow-md mb-6">
      ...
      <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
        ...
        <div className="dropdown dropdown-end">
          ...
          <ul tabIndex={0} className="dropdown-content z-[50] menu p-2 mt-2 shadow-xl bg-[var(--color-brand-surface)] border border-[var(--color-brand-border)] rounded-lg w-40 text-[var(--color-brand-text-secondary)] font-medium max-h-60 overflow-y-auto">
            <li>
              <a onClick={() => { setFilters({...filters, tag: 'all'}); document.activeElement?.blur(); }} className={`hover:bg-[var(--color-brand-dark)] hover:text-[var(--color-brand-text-primary)] ${filters.tag === 'all' ? 'bg-[var(--color-brand-dark)] text-[var(--color-brand-text-primary)]' : ''}`}>
                All
              </a>
            </li>
            {availableTags.map(t => (
              <li key={t}>
                <a onClick={() => { setFilters({...filters, tag: t}); document.activeElement?.blur(); }} className={`hover:bg-[var(--color-brand-dark)] hover:text-[var(--color-brand-text-primary)] ${filters.tag === t ? 'bg-[var(--color-brand-dark)] text-[var(--color-brand-text-primary)]' : ''}`}>
